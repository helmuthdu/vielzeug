...
/**
 * Reads the @js-temporal/polyfill UMD bundle from tempo's node_modules and wraps it
 * so `globalThis.Temporal` is available in the REPL sandbox. The UMD sets
 * `globalThis.temporal.Temporal` (lowercase namespace); tempo's IIFE expects a
 * top-level `Temporal` global, so the wrapper copies it into place after the UMD runs.
 * Returns empty string if the polyfill can't be found (e.g. in test fixtures).
 */
function readTemporalPolyfillSource(packagesDir = PACKAGES_DIR): string {
  const umdPath = join(packagesDir, 'tempo', 'node_modules', '@js-temporal/polyfill/dist/index.umd.js');
...
export const TEMPORAL_POLYFILL_SOURCE: string = ${quote(temporalPolyfillSource)};
