# Change Log - @vielzeug/familiar

This log was last generated on Wed, 09 Sep 2026 22:15:14 GMT and should not be manually modified.

## 3.0.0
Wed, 09 Sep 2026 22:15:14 GMT

### Breaking changes

- refactor!: focus Familiar on worker pools and ordered runBatch composition while hardening protocol validation, cancellation, queueing, streams, errors, and lifecycle behavior

## 2.2.0
Sun, 30 Aug 2026 13:16:08 GMT

### Minor changes

- refactor: extract PoolCore into _pool-core module. Remove FamiliarQueueFullError. Reorganize pool internals for capacity and idle waiting.

## 2.1.0
Wed, 26 Aug 2026 17:29:55 GMT

### Minor changes

- feat: remove static is() type guard from base error class; add sideEffects: false for tree-shaking.

## 2.0.2
Fri, 21 Aug 2026 16:02:58 GMT

### Patches

- fix: resolve TypeScript type errors in test files

## 2.0.1
Sun, 16 Aug 2026 09:15:40 GMT

_Version update only_

## 2.0.0
Mon, 10 Aug 2026 15:11:23 GMT

### Breaking changes

- refactor!: replace inline workers with module-worker pools

## 1.0.8
Thu, 06 Aug 2026 07:20:49 GMT

### Patches

- publish clean export metadata and classic TypeScript subpath mappings

## 1.0.7
Wed, 05 Aug 2026 16:48:52 GMT

_Version update only_

## 1.0.6
Sun, 26 Jul 2026 06:43:54 GMT

### Patches

- refactor(familiar): derive vite external list from package.json via readWorkspaceDeps() instead of hand-listing dependencies

## 1.0.5
Fri, 24 Jul 2026 05:28:41 GMT

### Patches

- chore: bump engines.node to >=22 to match .nvmrc/CLAUDE.md's Node 22 requirement

## 1.0.4
Tue, 14 Jul 2026 06:12:09 GMT

### Patches

- fix: rewrite workspace:* deps to real semver on publish (was shipping literal 'workspace:*' to npm, breaking installs outside this monorepo)

## 1.0.3
Tue, 07 Jul 2026 09:20:39 GMT

### Patches

- chore: declare minimum supported Node.js version (>=18) in package.json engines

## 1.0.2
Sun, 05 Jul 2026 05:52:18 GMT

### Patches

- fix(familiar): stale README/docs error-class references, bounded batch(ordered:false) buffering, timer-unref consistency, and coverage gaps

## 1.0.1
Fri, 03 Jul 2026 06:00:47 GMT

### Patches

- chore(familiar): rename internal _warn.ts to _dev.ts

## 1.0.0
Wed, 01 Jul 2026 16:10:37 GMT

### Breaking changes

- Initial public release

