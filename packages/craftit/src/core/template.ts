...
const clearAfterMarker = (marker: Comment) => {
...
const createNodes = (htmlString: string): Node[] => Array.from(parseHTML(htmlString).childNodes);
...
const insertNodes = (marker: Comment, nodes: Node[], before: Node | null) => {
...
export const applyAttrBinding = (el: HTMLElement, binding: AttrBinding, registerCleanup: RegisterCleanup) => {
  const update = (v: unknown) => {
    ...
    // For declared props, attribute reflection is controlled by prop metadata.
    ...
    const isStructuredValue = Array.isArray(v) || (typeof v === 'object' && v !== null);
    const parsedValue = isStructuredValue
      ? v
      : meta.parse(binding.mode === 'bool' ? (v ? '' : null) : v == null || v === false ? null : String(v));
...
/** @internal — resets the marker counter. Used by _resetCounters in test/test.ts. */
