...
export type SetupResult = string | HTMLResult;
...
// ─── CraftitBaseElement ───────────────────────────────────────────────────────
class CraftitBaseElement extends HTMLElement {
  ...
  private _runSetup(): void {
    ...
    try {
      ...
      // MutationObserver to simulate attributeChangedCallback for dynamic defineProps
      ...
      const res = (this.constructor as typeof CraftitBaseElement)._setup({
        host: this,
        shadow: this.shadow,
      });
 ...
 * @param setup - Setup function called for each instance. Receives a {@link SetupContext}.
 * @param options - Configuration for the custom element.
 * @returns The custom element tag name.
 */
export function define(name: string, setup: (ctx: SetupContext) => SetupResult, options: DefineOptions = {}): string {
  if (!name) throw new Error('[craftit:E4] define(name, ...) requires a tag name');

  if (typeof setup !== 'function') throw new Error('[craftit:E5] define(...) requires a setup function');

  if (typeof options !== 'object') {
    throw new Error('[craftit:E6] define options must be an object');
  }
