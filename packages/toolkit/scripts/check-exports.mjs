...
const getToolkitSections = async () => {
...
const main = async () => {
  ...
  const sections = await getToolkitSections();
  const expectedExportKeys = ['.', ...sections.map((section) => `./${section}`)].sort();
  const actualExportKeys = Object.keys(packageJson.exports).sort();

  assertEqualSet('Toolkit package exports do not match src sections', actualExportKeys, expectedExportKeys);
  ...
  for (const [field, expectedValue] of Object.entries(expectedRootExport)) {
    if (packageJson.exports['.']?.[field] !== expectedValue) {
      throw new Error(
        `Toolkit export "." has unexpected ${field}: expected ${expectedValue}, received ${packageJson.exports['.']?.[field]}`,
      );
    }
  }
  ...
  for (const section of sections) {
    ...
    for (const [field, expectedValue] of Object.entries(expectedEntry)) {
      if (exportEntry?.[field] !== expectedValue) {
        throw new Error(
          `Toolkit export "./${section}" has unexpected ${field}: expected ${expectedValue}, received ${exportEntry?.[field]}`,
        );
      }
    }
    ...
    assertEqualSet(
      `Toolkit subpath barrel drift in src/${section}/index.ts`,
      actualBarrelModules,
  ...
  assertEqualSet('Toolkit root barrel drift in src/index.ts', actualRootModules, expectedRootBarrelModules);
};
