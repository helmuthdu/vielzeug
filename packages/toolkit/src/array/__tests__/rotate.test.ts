...
describe('rotate', () => {
  ...
  it('should throw TypeError if input is not an array', () => {
    // @ts-expect-error
    expect(() => rotate(null, 1)).toThrow(TypeError);
    // @ts-expect-error
    expect(() => rotate(undefined, 1)).toThrow(TypeError);
    // @ts-expect-error
    expect(() => rotate({}, 1)).toThrow(TypeError);
  });

  it('should throw TypeError if positions is not a number', () => {
    // @ts-expect-error
    expect(() => rotate([1, 2, 3], null)).toThrow(TypeError);
    // @ts-expect-error
    expect(() => rotate([1, 2, 3], undefined)).toThrow(TypeError);
    // @ts-expect-error
    expect(() => rotate([1, 2, 3], {})).toThrow(TypeError);
  });
});
