/**
 * CEM Analyzer plugin for the Refine `define<Props>(TAG, config)` factory pattern.
 *
 * The standard CEM analyzer only detects `customElements.define()` calls and class-based
 * custom elements. Refine components are registered via a functional `define<Props>(TAG, {...})`
...
export function refineCemPlugin() {
  ...
  return {
     ...
     * (e.g. ore-tr/ore-th/ore-td in table.ts), merge the JSDoc data we collected
    ...
    analyzePhase({ ts, node, moduleDoc, context }) {
      ...
      //   customElements.define('ore-tr', class extends HTMLElement {});
      //   if (!customElements.get('ore-tr')) customElements.define('ore-tr', ...);
      {
        ...
        if (ts.isExpressionStatement(node)) {
          ...
          if (
            ...
            expr.arguments[0].text.startsWith('ore-')
          ) {
            defineCallNode = node;
          }
          ...
          if (
            ...
            thenExpr.arguments[0].text.startsWith('ore-')
          ) {
            defineCallNode = node;
          }
        }
      ...
      // We look for: export const FOO_TAG = 'ore-foo' as const;
      ...
      if (ts.isExpressionStatement(node)) {
        ...
        if (
          ...
          if (ts.isStringLiteral(tagArg)) {
            ...
            // Walk the source file to find `const IDENTIFIER = 'ore-...' as const`
            const sourceFile = node.getSourceFile();
