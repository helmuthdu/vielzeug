...
describe('createOverlayControl', () => {
  ...
  it('closes on outside pointerdown and keeps open on boundary/panel pointerdown', () => {
    const openState = signal(false);
    ...
    overlay.open('click');
    pointerdown(insideBoundary);
    expect(openState.value).toBe(true);

    pointerdown(insidePanel);
    expect(openState.value).toBe(true);
  ...
  it('uses event.target fallback when composedPath is unavailable', () => {
    ...
    const pd = new PointerEvent('pointerdown', { bubbles: true, composed: true });

    Object.defineProperty(pd, 'composedPath', { value: undefined });
    outside.dispatchEvent(pd);
