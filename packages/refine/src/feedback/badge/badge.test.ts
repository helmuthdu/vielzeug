...
describe('ore-badge', () => {
  ...
  describe('Dot Mode', () => {
    it('renders dot when dot attribute set', async () => {
      fixture = await mount('ore-badge', { attrs: { dot: '' } });
  ...
  describe('Colors', () => {
    for (const color of ['primary', 'success', 'warning', 'error', 'info']) {
      it(`applies ${color} color`, async () => {
        fixture = await mount('ore-badge', { attrs: { color } });
  ...
  describe('Variants', () => {
    for (const variant of ['solid', 'flat', 'bordered', 'outline', 'frost']) {
      it(`applies ${variant} variant`, async () => {
        fixture = await mount('ore-badge', { attrs: { variant } });
...
describe('ore-badge accessibility', () => {
  ...
  describe('Count Visibility', () => {
    it('count is visible in DOM', async () => {
      fixture = await mount('ore-badge', { attrs: { count: '7' } });

      expect(fixture.query('.badge')?.textContent).toContain('7');
    });

    it('max plus is visible in DOM', async () => {
      fixture = await mount('ore-badge', { attrs: { count: '150', max: '99' } });
  ...
  describe('Accessibility', () => {
    it('passes axe checks with default rendering', async () => {
      fixture = await mount('ore-badge', { html: '<button>Inbox</button>' });
