...
describe('ore-password-strength', () => {
  ...
  describe('Core Functionality', () => {
    it('renders meter structure with four segments', async () => {
      fixture = await mount('ore-password-strength');
    ...
    it('activates segments based on computed score from value', async () => {
      fixture = await mount('ore-password-strength', { attrs: { value: 'Abcdef12!' } });

      expect(fixture.queryAll('.segment.active').length).toBe(3);
    });

    it('uses score override when provided', async () => {
      fixture = await mount('ore-password-strength', { attrs: { score: '4', value: 'abc' } });
    ...
    it('normalizes out-of-range score to 4', async () => {
      fixture = await mount('ore-password-strength', { attrs: { score: '10' } });

      expect(fixture.queryAll('.segment.active').length).toBe(4);
    });

    it('uses value scoring when score is -1 (no override)', async () => {
      fixture = await mount('ore-password-strength', { attrs: { score: '-1', value: 'Tr0ub4dor&3Abc' } });

      expect(fixture.queryAll('.segment.active').length).toBe(4);
    });

    it('supports custom labels array', async () => {
      fixture = await mount('ore-password-strength', { attrs: { score: '2' } });
  ...
  describe('Accessibility & WAI-ARIA', () => {
    ...
    it('supports custom accessible label', async () => {
      fixture = await mount('ore-password-strength', { attrs: { label: 'Account password strength' } });
  ...
  describe('Edge Cases & Error Handling', () => {
    ...
    it('reacts to dynamic value changes', async () => {
      fixture = await mount('ore-password-strength', { attrs: { value: 'abc' } });
