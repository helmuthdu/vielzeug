...
describe('ore-navbar', () => {
  ...
  it('turns floating+sticky into sticky mode after scroll threshold', async () => {
    ...
    try {
      ...
      fixture = await mount('ore-navbar', { attrs: { floating: '', 'scroll-threshold': '20', sticky: '' } });
      expect(fixture.element.getAttribute('data-mode')).toBe('floating');
  ...
  it('respects invalid scroll-threshold by falling back to the default (80px)', async () => {
    ...
    try {
      ...
      fixture = await mount('ore-navbar', { attrs: { floating: '', 'scroll-threshold': 'abc', sticky: '' } });
...
describe('ore-navbar-item', () => {
  ...
  it('renders an anchor when href is present', async () => {
    fixture = await mount('ore-navbar-item', { attrs: { href: '/dashboard' } });

    expect(fixture.query('a.item')).toBeTruthy();
  });

  it('renders a button when href is absent', async () => {
    fixture = await mount('ore-navbar-item');
  ...
  it('applies aria-current=page when active', async () => {
    fixture = await mount('ore-navbar-item', { attrs: { active: '', href: '/dashboard' } });

    expect(fixture.query('a.item')?.getAttribute('aria-current')).toBe('page');
  });

  it('keeps semantic text content accessible', async () => {
    fixture = await mount('ore-navbar-item', { html: 'Dashboard' });
  ...
  describe('Accessibility', () => {
    it('passes axe checks', async () => {
      fixture = await mount('ore-navbar', {
        ...
        html: '<ore-navbar-item href="/">Home</ore-navbar-item>',
      });
