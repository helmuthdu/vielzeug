...
describe('ore-avatar', () => {
  ...
  describe('Rendering', () => {
    it('renders avatar container element', async () => {
      fixture = await mount('ore-avatar');

      expect(fixture.query('.avatar')).toBeTruthy();
    });

    it('renders image element when src is provided', async () => {
      fixture = await mount('ore-avatar', { attrs: { alt: 'Jane Doe', src: '/photo.jpg' } });
    ...
    it('explicit initials prop overrides alt-derived initials', async () => {
      fixture = await mount('ore-avatar', { attrs: { alt: 'Jane Doe', initials: 'XY' } });

      expect(fixture.query('.initials')?.textContent?.trim()).toBe('XY');
    });

    it('initials prop is truncated to 2 characters', async () => {
      fixture = await mount('ore-avatar', { attrs: { initials: 'ABC' } });
  ...
  describe('Status Indicator', () => {
    ...
    it('status dot has data-status attribute', async () => {
      fixture = await mount('ore-avatar', { attrs: { status: 'busy' } });
...
describe('ore-avatar accessibility', () => {
  let fixture: Fixture<HTMLElement>;
  ...
  describe('Image Alt Text', () => {
    it('avatar wrapper has role="img" when alt text is provided', async () => {
      fixture = await mount('ore-avatar', { attrs: { alt: 'Profile photo of Alice' } });
  ...
  describe('Status Indicator', () => {
    ...
    it('avatar label includes status when alt and status are both set', async () => {
      fixture = await mount('ore-avatar', { attrs: { alt: 'Jane Doe', status: 'online' } });
...
describe('ore-avatar-group', () => {
  ...
  describe('Accessibility', () => {
    it('passes axe checks', async () => {
      fixture = await mount('ore-avatar', { attrs: { name: 'Jane Doe' } });
