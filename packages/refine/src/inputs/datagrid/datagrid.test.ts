...
describe('ore-datagrid', () => {
  ...
  // ── Declarative ore-column ─────────────────────────────────────────────────────

  describe('Declarative ore-column children', () => {
    it('renders columns from ore-column children when no columns prop is set', async () => {
      fixture = await mount('ore-datagrid', {});

      const el = fixture.element as HTMLElement & { rows: User[] };

      el.innerHTML = `
        <ore-column key="name" label="Name"></ore-column>
    ...
    it('JS columns prop wins over declarative children when both are set', async () => {
      ...
      el.innerHTML = '<ore-column key="name" label="FromHTML"></ore-column>';
    ...
    it('marks sortable columns from ore-column[sortable]', async () => {
      fixture = await mount('ore-datagrid', {});

      const el = fixture.element as HTMLElement & { rows: User[] };

      el.innerHTML = '<ore-column key="name" label="Name" sortable></ore-column>';
      el.rows = ROWS;
      await new Promise((r) => setTimeout(r, 0));
    ...
    it('setting columns=[] explicitly clears declarative children', async () => {
      ...
      el.innerHTML = '<ore-column key="name" label="Name"></ore-column>';
      el.rows = ROWS;
      await new Promise((r) => setTimeout(r, 0));
    ...
    it('reacts to dynamically added ore-column children', async () => {
      fixture = await mount('ore-datagrid', {});
      ...
      const col = document.createElement('ore-column');
  ...
  // ── ore-column attribute reactivity ──────────────────────────────────────

  describe('ore-column attribute reactivity', () => {
    it('updating label attribute on ore-column updates the header cell', async () => {
      fixture = await mount('ore-datagrid', {});
      ...
      const col = el.querySelector('ore-column')!;
    ...
    it('updating sortable attribute on ore-column adds sort button', async () => {
      fixture = await mount('ore-datagrid', {});
  ...
  // ── C3: ore-column validation warnings ───────────────────────────────────

  describe('C3: ore-column connectedCallback validation', () => {
    it('warns when ore-column is missing key attribute', async () => {
      ...
      expect(spy).toHaveBeenCalledWith(expect.stringContaining('[@vielzeug/refine]'));

      col.remove();
      spy.mockRestore();
    });

    it('warns when ore-column is missing label attribute', async () => {
      const spy = vi.spyOn(console, 'warn').mockImplementation(() => {});
    ...
    it('does not warn when ore-column has both key and label', async () => {
      const spy = vi.spyOn(console, 'warn').mockImplementation(() => {});
      ...
      expect(spy).not.toHaveBeenCalledWith(expect.stringContaining('[@vielzeug/refine]'));
  ...
  describe('Accessibility', () => {
    it('passes axe checks', async () => {
      fixture = await mount('ore-datagrid', {
        ...
        html: `
          <ore-datagrid-col key="name" label="Name"></ore-datagrid-col>
          <ore-datagrid-col key="age" label="Age"></ore-datagrid-col>
        `,
      });
