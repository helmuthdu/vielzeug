...
describe('ore-select', () => {
  ...
  describe('Rendering', () => {
    ...
    it('renders helper text when set', async () => {
      ...
      expect(fixture.query('ore-input.trigger')?.getAttribute('helper')).toBe('Choose one');
    });

    it('passes error prop to ore-input', async () => {
      fixture = await mount('ore-select', {
  ...
  describe('Selection', () => {
    it('keeps chip row hidden in single mode before and after selection', async () => {
      ...
      expect(chipsRow?.hasAttribute('hidden')).toBe(true);
      expect(fixture.queryAll('ore-chip').length).toBe(0);
    });
    ...
    it('skips disabled options when clicked', async () => {
      fixture = await mount('ore-select', { html: SELECT_OPTIONS_WITH_DISABLED });
  ...
  describe('Error State', () => {
    it('passes error prop to ore-input trigger', async () => {
      fixture = await mount('ore-select', {
      ...
      expect(fixture.query('ore-input.trigger')?.getAttribute('error')).toBe('Required');
    });
  });
  ...
  describe('Form Integration', () => {
    ...
    it('renders chips after csv value update in multiple mode', async () => {
      ...
      expect(fixture.queryAll('ore-chip').length).toBe(2);
    });
...
describe('ore-select accessibility', () => {
  let fixture: Fixture<HTMLElement>;
  ...
  describe('Semantic Structure', () => {
    it('trigger has role="combobox"', async () => {
      ...
      expect(fixture.query('ore-input.trigger')?.getAttribute('role')).toBe('combobox');
    });
    ...
    it('error prop is passed to ore-input trigger', async () => {
      fixture = await mount('ore-select', {
        attrs: { error: 'Selection required' },
        html: SELECT_OPTIONS,
      });
  ...
  describe('Focus Management', () => {
    it('trigger is keyboard focusable (tabindex="0")', async () => {
      ...
      expect(fixture.query('ore-input.trigger')?.getAttribute('tabindex')).toBe('0');
    });
    ...
    it('trigger is not focusable when disabled (tabindex="-1")', async () => {
      ...
      expect(fixture.query('ore-input.trigger')?.getAttribute('tabindex')).toBe('-1');
    });
  });
