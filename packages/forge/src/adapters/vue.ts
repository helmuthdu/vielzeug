 ...
 * import { init } from '@vielzeug/forge/vue';
 *
...
function requireDeps(): { onScopeDispose: OnScopeDisposeFn; shallowRef: ShallowRefFn } {
  if (!_shallowRef || !_onScopeDispose) {
    throw new Error(
      '[forge/vue] Call init({ shallowRef, onScopeDispose }) once at your app entry before using form composables.',
    );
  }
 ...
 *
 * // useMyForm.ts
 * import { createForm, useFormState } from '@vielzeug/forge/vue';
 *
 ...
 * @example
 * ```ts
 * import { createForm, useField } from '@vielzeug/forge/vue';
 *
 * export function useMyForm() {
 *   const form = createForm({ defaultValues: { email: '' } });
 ...
 * @example
 * ```ts
 * import { createForm, useFormValues } from '@vielzeug/forge/vue';
 *
 * export function useMyForm() {
