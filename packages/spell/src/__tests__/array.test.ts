...
describe('s.array()', () => {
  it('accepts arrays, rejects non-arrays', () => {
    expect(s.array(s.string()).parse(['a', 'b'])).toEqual(['a', 'b']);
    expect(s.array(s.string()).parse([])).toEqual([]);
    expect(() => s.array(s.string()).parse('not an array')).toThrow('Expected array');
  });

  it('validates each item and puts the index in the path', () => {
    const result = s.array(s.string()).safeParse([1, 2]);
  ...
  it('min / max / length', () => {
    expect(s.array(s.number()).min(2).parse([1, 2])).toEqual([1, 2]);
    expect(() => s.array(s.number()).min(2).parse([1])).toThrow('Must have at least 2 items');
    expect(() => s.array(s.number()).max(2).parse([1, 2, 3])).toThrow('Must have at most 2 items');
    expect(() => s.array(s.number()).length(2).parse([1])).toThrow('Must have exactly 2 items');
  });
...
describe('array — field-level transforms', () => {
  ...
  it('trim() inside array items preserves trimmed values', () => {
    expect(s.array(s.string().trim()).parse(['  a  ', '  b  '])).toEqual(['a', 'b']);
  });

  it('default() inside array fills missing item values', () => {
    expect(s.array(s.string().default('x')).parse([undefined, 'hello'])).toEqual(['x', 'hello']);
  });
});
...
describe('array parseAsync — optional / nullable / catch', () => {
  ...
  it('array.nullable().parseAsync(null) returns null', async () => {
    expect(await s.array(s.string()).nullable().parseAsync(null)).toBeNull();
  });
