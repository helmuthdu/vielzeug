  ...
  for (let i = 0; i < segments.length; i += 1) {
    ...
    if (segment === '*') {
      if (i !== segments.length - 1) {
        throw new Error(`[route] Wildcard "*" must be the final segment in path: ${path}`);
      }
    ...
    if (segment.startsWith(':') && segment.endsWith('*')) {
      if (i !== segments.length - 1) {
        throw new Error(`[route] Wildcard param must be final segment in path: ${path}`);
      }
      ...
      if (!/^\w+$/.test(name)) {
        throw new Error(
          `[route] Invalid param name ":${name}" in path "${path}". Param names must only contain word characters (a–z, A–Z, 0–9, _).`,
        );
      }
...
export function buildUrl(base: string, pattern: string, params: RouteParams = {}, query?: ResolvedQueryParams): string {
  let path = pattern.replace(/:(\w+)(\*)?/g, (_match, key: string, isWildcard: string) => {
    ...
    if (value == null) throw new Error(`[route] Missing path param: ${key}`);
