...
const MISSING_TZ = '[timit] This operation requires a timezone. Pass options.tz or use a ZonedDateTime input.';
const MISMATCH_ZONES =
  '[timit] Comparison received ZonedDateTime inputs with different time zones. Pass options.tz explicitly.';
...
describe('parseLocal', () => {
  ...
  it('throws a descriptive error for invalid input', () => {
    expect(() => parseLocal('not-a-date')).toThrow(
      '[timit] Invalid local date/time string. Expected an ISO 8601 date or date-time string',
    );
  });
});
...
describe('difference', () => {
  it('computes duration between two instants', () => {
  ...
  it('throws when no timezone can be inferred (both inputs are Temporal.Instant)', () => {
    const a = Temporal.Instant.from('2026-03-21T10:00:00Z');
    const b = Temporal.Instant.from('2026-03-21T12:00:00Z');

    expect(() => difference(a, b, { largestUnit: 'hour' })).toThrow(MISSING_TZ);
  });
...
describe('formatHuman', () => {
  ...
  it('supports date-only and time-only presets', () => {
    const dateOnly = formatHuman(Temporal.Instant.from('2026-03-21T10:15:30Z'), {
      locale: 'en-GB',
      pattern: 'date-only',
      tz: 'UTC',
    });
    const timeOnly = formatHuman(Temporal.Instant.from('2026-03-21T10:15:30Z'), {
      locale: 'en-GB',
      pattern: 'time-only',
      tz: 'UTC',
    });
  ...
  it('infers timezone from ZonedDateTime input', () => {
    ...
    expect(formatHuman(input, { locale: 'en-GB', pattern: 'date-only' })).toContain('21/03/2026');
  });
  ...
  it('returns stable output across repeated calls with same options', () => {
    ...
    expect(formatHuman(input, options)).toBe(formatHuman(input, options));
  });

  it('honours intl.timeZone when options.tz is absent', () => {
    // 10:00 UTC = 11:00 in Europe/Berlin (UTC+1 in March)
    const input = Temporal.Instant.from('2026-03-21T10:00:00Z');
    const withTzOption = formatHuman(input, { intl: { timeStyle: 'short' }, locale: 'en-GB', tz: 'Europe/Berlin' });
    const withIntlTimeZone = formatHuman(input, {
  ...
  it('throws for plain input without tz', () => {
    expect(() => formatHuman(parseLocal('2026-03-21T10:15:30'))).toThrow(MISSING_TZ);
  });

  it('formats a PlainDate with an explicit timezone', () => {
    const date = Temporal.PlainDate.from('2026-03-21');
    const result = formatHuman(date, { locale: 'en-GB', pattern: 'date-only', tz: 'UTC' });
  ...
  it('formats a PlainDateTime with an explicit timezone', () => {
    ...
    const result = formatHuman(dt, { locale: 'en-GB', pattern: 'short', tz: 'UTC' });
...
describe('parseDuration', () => {
  ...
  it('throws for invalid input', () => {
    expect(() => parseDuration('not-a-duration')).toThrow('[timit] Invalid duration input.');
  });
