...
function durationFallback(value: DurationValue): string {
 ...
 * or an object with a `locale` property (e.g. an `I18n` instance) for reactive binding.
 */
export function createFormatter(
  source: string | { readonly locale: string } | { getSnapshot(): { locale: string } },
): Formatter {
  ...
  return {
    ...
    duration(value, options) {
      ...
      if (!IntlExt.DurationFormat) return durationFallback(value);
