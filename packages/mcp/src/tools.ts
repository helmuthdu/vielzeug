...
interface ToolContext {
  ...
  /** Null when /block was not built before data generation. */
...
const BLOCK_UNAVAILABLE =
  'Block component metadata is unavailable in this snapshot. If using the monorepo, build /block first then run prepare:data. Published releases include this data automatically.';

const listComponentsTool: ToolDefinition = {
  description: 'List all /block web component tags from bundled CEM metadata.',
  inputSchema: { properties: {}, type: 'object' },
  name: 'list-components',
  run(_args, context) {
    if (!context.components) return error(BLOCK_UNAVAILABLE);
...
const getComponentTool: ToolDefinition = {
  description: 'Get a full /block component CEM declaration by HTML tag name.',
  inputSchema: {
    properties: {
...
export function registerTools(server: Server, data: BundledData): void {
  const blockPkg = data.packages.find((p) => p.slug === 'block');

  const context: ToolContext = {
    bySlug: new Map(data.packages.map((pkg) => [pkg.slug, pkg])),
    components: blockPkg && blockPkg.components.length > 0 ? blockPkg.components : null,
