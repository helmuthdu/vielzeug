...
const BUILDIT_UNAVAILABLE =
  'Block component metadata is unavailable in this snapshot. Build /block before generating MCP bundled data.';
...
const SLUG_INPUT = {
  properties: { packageSlug: { description: 'Package folder name, e.g. "ripple"', minLength: 1, type: 'string' } },
...
function getBuilditComponents(context: ToolContext): CemDeclaration[] | null {
  const block = context.bySlug.get('block');

  if (!block || block.components.length === 0) {
    return null;
  }
...
const TOOLS = {
  'get-component': {
    description: 'Get a full block component declaration by HTML tag name.',
    inputSchema: {
      properties: {
  ...
  'list-components': {
    description: 'List all /block web component tags from bundled component metadata.',
