...
const REGEN_CMD = 'pnpm --dir packages/mcp run prepare:data';
...
export function loadData(): BundledData {
  ...
  try {
    ...
    if (code === 'ENOENT') {
      throw new Error(
        `Bundled MCP data not found at ${DATA_FILE}. In the monorepo run ${REGEN_CMD}; for standalone installs, reinstall /mcp to restore packaged data.`,
        { cause: error },
      );
    }
