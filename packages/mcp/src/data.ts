...
export function loadData(): BundledData {
  try {
    ...
    if (code === 'ENOENT') {
      throw new Error(
        `Bundled MCP data not found at ${DATA_FILE}. In the monorepo run \`pnpm --dir packages/mcp run prepare:data\`; for standalone installs, reinstall /mcp to restore packaged data.`,
        { cause: error },
      );
    }
    ...
    if (error instanceof SyntaxError) {
      throw new Error(
        `Bundled MCP data at ${DATA_FILE} is malformed. Regenerate it with \`pnpm --dir packages/mcp run prepare:data\`.`,
        { cause: error },
      );
    }
