...
function readBuilditDeclarations(repoRoot: string): CemDeclaration[] {
...
function processPackage(repoRoot: string, project: RushProject, builditComponents: CemDeclaration[]): BundledPackage {
      ...
      if (incremental) {
        ...
        if (hashCache[slug] === currentHash) {
          ...
          if (cached) {
            ...
            // Always refresh block components — they come from dist/, not source files
