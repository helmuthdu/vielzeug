...
type CheckContext = { addIssue: (issue: Omit<Issue, 'path'> & { path?: (string | number)[] }) => void };

/**
 * The return type of a check() callback.
 *
 ...
 */
type CheckFnResult = void | null | undefined | boolean | string;
...
export class Schema<Output = unknown, Input = Output> {
  ...
  parse(value: unknown): Output {
    return this._withCatch(() => {
      ...
      const hasInvalidType = core.issues.some((i) => i.code === ErrorCode.invalid_type);
  ...
  check(fn: (value: Output, ctx: CheckContext) => CheckFnResult | Promise<CheckFnResult>): this {
    const validator: ValidateFn = (value, path) => {
      ...
      const ctx: CheckContext = {
        addIssue: (issue) => {
          ctxIssues.push({ ...issue, path: issue.path === undefined ? path : issue.path });
        },
      };
