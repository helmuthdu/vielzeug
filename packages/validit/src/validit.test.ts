...
describe('validit', () => {
  ...
  describe('v.enum()', () => {
    ...
    it('should validate mixed enums', () => {
      const schema = v.enum('active', 1, 'inactive', 0);
      expect(schema.parse('active')).toBe('active');
      expect(schema.parse(1)).toBe(1);
  ...
  describe('Complex schemas', () => {
    it('should validate complex nested structures', () => {
      const schema = v.object({
        ...
        roles: v.array(v.enum('admin', 'user', 'guest')).min(1),
        settings: v.object({
  ...
  describe('Convenience schemas', () => {
    ...
    it('should validate positive integers with v.int().positive()', () => {
      const schema = v.int().positive();
      expect(schema.parse(5)).toBe(5);
      expect(() => schema.parse(-5)).toThrow();
      expect(() => schema.parse(3.14)).toThrow('Must be an integer');
    });

    it('should validate negative integers with v.int().negative()', () => {
      const schema = v.int().negative();
      expect(schema.parse(-5)).toBe(-5);
      expect(() => schema.parse(5)).toThrow();
      expect(() => schema.parse(-3.14)).toThrow('Must be an integer');
    });
  ...
  describe('Non-empty validation patterns', () => {
    it('should reject empty strings with min(1)', () => {
      const schema = v.string().min(1);
      expect(() => schema.parse('')).toThrow('Must be at least 1 characters');
      expect(schema.parse('hello')).toBe('hello');
    });

    it('should reject null/undefined automatically', () => {
      const schema = v.string();
      expect(() => schema.parse(undefined)).toThrow('Expected string');
      expect(() => schema.parse(null)).toThrow('Expected string');
    });

    it('should support custom error messages for min(1)', () => {
      const schema = v.string().min(1, 'Name is required');
    ...
    it('should work in objects', () => {
      ...
      expect(() => schema.parse({ email: 'john@example.com', name: '' })).toThrow('Must be at least 1 characters');
      expect(() => schema.parse({ email: '', name: 'John' })).toThrow('Email is required');
      expect(() => schema.parse({ name: 'John' })).toThrow('Expected string'); // email is undefined
    });
