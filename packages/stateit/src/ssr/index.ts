 ...
 * ```ts
 * // frameworks/hono.ts
 * import { createAsyncProvider, setTrackingProvider } from '@vielzeug/stateit/ssr';
 *
...
export const createAsyncProvider = (): TrackingProvider => {
  ...
  const { AsyncLocalStorage } = require('async_hooks') as { AsyncLocalStorage: new <T>() => AsyncLocalStorage<T> };
