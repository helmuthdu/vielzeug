...
[![npm version](https://img.shields.io/npm/v//machine.svg)](https://npmjs.com/package//machine)
[![npm downloads](https://img.shields.io/npm/dm//machine.svg)](https://npmjs.com/package//machine)
[![TypeScript support](https://img.shields.io/badge/TypeScript-5.0%2B-blue.svg)](#)

A production-ready finite state machine with typed events, reactive state, async invokes, persistence, and debugging—built on `/ripple` with zero external dependencies.

## Why Machine?

- **Type-safe** — Discriminated event unions with full TypeScript inference
- **Reactive** — State and context are `/ripple` signals
...
- **Zero deps** — Depends only on `/ripple` (peer)

## Quick Start

```ts
import { defineMachine, interpret, assign } from '/machine';
...
| **Reactive state** | Signals from `/ripple` |
...
- **[Full Guide](https://vielzeug.dev/machine/)** — Complete reference and patterns
- **[API Reference](https://vielzeug.dev/machine/api/)** — Types and functions
- **[Usage Guide](https://vielzeug.dev/machine/usage/)** — Common patterns and recipes
- **[Examples](https://vielzeug.dev/machine/examples/)** — Real-world use cases

## Installation

```sh
npm install /machine /ripple
```

**Note:** `/ripple` is a peer dependency and must be installed separately.

## TypeScript

Machine requires TypeScript 5.0+. Install globally or locally:
