...
describe("bit-file-input", () => {
	...
	describe("Rendering", () => {
		...
		it("renders label when provided", async () => {
			...
			expect(fixture.query('[part="label"]')?.textContent?.trim()).toBe(
				"Upload files",
			);
		});
		...
		it("renders helper text when provided", async () => {
			...
			expect(fixture.query('[part="helper"]')?.textContent?.trim()).toBe(
				"Max 5MB",
			);
		});
		...
		it("renders error text when provided", async () => {
			...
			expect(fixture.query('[part="error"]')?.textContent?.trim()).toBe(
				"File too large",
			);
		});
	...
	describe("Props", () => {
		...
		it("applies accept attribute to hidden input", async () => {
			...
			expect(fixture.query('input[type="file"]')?.getAttribute("accept")).toBe(
				".pdf,.docx",
			);
		});
		...
		it("applies multiple attribute", async () => {
			...
			expect(
				fixture.query('input[type="file"]')?.hasAttribute("multiple"),
			).toBe(true);
		});
		...
		it("applies required attribute to input", async () => {
			...
			expect(
				fixture.query('input[type="file"]')?.hasAttribute("required"),
			).toBe(true);
		});
