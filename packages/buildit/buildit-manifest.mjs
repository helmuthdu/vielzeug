...
export function readBuilditPackageJson() {
...
export function createBuilditExports(exportsField = {}) {
  return {
...
export function syncComponentExports() {
  const packageJson = readBuilditPackageJson();
  const previousContent = readFileSync(packageJsonUrl, 'utf8');

  packageJson.exports = createBuilditExports(packageJson.exports ?? {});
...
function printUsageAndExit() {
  processRef?.stderr.write('Usage: node ./buildit-manifest.mjs <check-exports|sync-exports>\n');
...
if (import.meta.url === new URL(processRef?.argv[1] ?? '', 'file:').href) {
  ...
  if (command === 'check-exports') {
    ...
    processRef?.stdout.write(`buildit component manifest verified for ${count} component exports.\n`);
  } else if (command === 'sync-exports') {
    const { changed, count } = syncComponentExports();

    processRef?.stdout.write(
      changed
        ? `Synced ${count} buildit component exports in package.json.\n`
        : `Buildit package exports already in sync for ${count} components.\n`,
    );
  } else {
    printUsageAndExit();
  }
