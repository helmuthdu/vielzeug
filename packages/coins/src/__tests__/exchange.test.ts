...
describe('exchange', () => {
  ...
  describe('error handling', () => {
    ...
    it('throws RangeError for invalid target currency (rate.to)', () => {
      const rate: ExchangeRate = { from: 'USD', rate: '0.85', to: 'FAKE' };

      expect(() => exchange(money('100.00', 'USD'), rate)).toThrow(RangeError);
      expect(() => exchange(money('100.00', 'USD'), rate)).toThrow('Invalid ISO 4217 currency code');
    });
  });
});
