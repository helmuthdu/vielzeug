...
describe('prototype chain safety', () => {
  test('set() throws for a top-level __proto__ key', () => {
    ...
    expect(() => form.set('__proto__' as never, 'x')).toThrow(
      "[formit] Unsafe key '__proto__': segments __proto__, constructor, and prototype are reserved.",
    );
  });
  ...
  test('set() throws for __proto__ as an inner dot-notation segment', () => {
    ...
    expect(() => form.set('a.__proto__.b' as never, 'x')).toThrow('[formit] Unsafe key');
  });

  test('set() throws for a top-level constructor key', () => {
    const form = createForm({ defaultValues: {} as Record<string, unknown> });

    expect(() => form.set('constructor' as never, 'x')).toThrow('[formit] Unsafe key');
  });

  test('set() throws for a top-level prototype key', () => {
    const form = createForm({ defaultValues: {} as Record<string, unknown> });

    expect(() => form.set('prototype' as never, 'x')).toThrow('[formit] Unsafe key');
  });

  test('setError() throws for an unsafe key', () => {
    const form = createForm({ defaultValues: {} as Record<string, unknown> });

    expect(() => form.setError('__proto__' as never, 'bad message')).toThrow('[formit] Unsafe key');
  });

  test('setValidator() throws for an unsafe key', () => {
    const form = createForm({ defaultValues: {} as Record<string, unknown> });

    expect(() => form.setValidator('__proto__' as never, () => undefined)).toThrow('[formit] Unsafe key');
  });
