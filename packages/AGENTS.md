...
- Zero third-party runtime dependencies; inter-package `@vielzeug/*` dependencies are allowed and use `workspace:*`. **Exception: `refine` bundles `lucide`** — see its local contract. `refine`, `prism`, and `ore` also use `axe-core` as a devDependency for accessibility testing; it is not bundled. See `.ai/core/conventions.md` for the full documented exceptions list.
