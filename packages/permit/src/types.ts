...
export type Permit<TAction extends string = string, TData = unknown> = {
  ...
  rulesFor(principal: Principal, resource: string): PermitRule<TAction, TData>[];
};

/**
 * Bound permit view: decision-only methods scoped to a principal.
 * @internal
 */
export type BoundPermit<TAction extends string = string, TData = unknown> = {
  ...
  forUser(principal: UserPrincipal): BoundPermit<TAction, TData>;
  rulesFor(resource: string): PermitRule<TAction, TData>[];
};
