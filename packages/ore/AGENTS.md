...
- **Multiple public entry points** — `.`, `./directives`, `./forms`, `./observers`, `./testing`. Each maps to a `source`/`import`/`require`/`types` block in `package.json` `exports`. When adding or moving an entry point, update `package.json` `exports` and `vite.config.ts`'s `entry` map to match.
