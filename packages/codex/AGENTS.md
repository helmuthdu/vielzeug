...
- MCP tools are defined in `src/tools.ts`. Keep the tool list in sync with `README.md` when tools change.
...
- When adding an MCP tool: implement in `src/tools.ts`, cover it in `src/__tests__/`, and update the tool list in `README.md`.
