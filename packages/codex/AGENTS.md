...
- Entry points: `src/cli.ts` (CLI, run as `node dist/cli.js`), `src/index.ts` (server), `src/http.ts` (HTTP transport via `--port`).
- MCP tools are defined in `src/tools.ts`; resources in `src/resources.ts`. Keep the tool list in sync with `README.md` and `.devin/rules/conventions.md` ("AI integration") when tools change.
