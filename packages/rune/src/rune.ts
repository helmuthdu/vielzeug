...
function failInvalidArgs(message: string): never {
  throw new TypeError(`[rune] ${message}`);
}
...
function resolveRemote(
  remote: RuneOptions['remote'],
  inherited?: RuneConfig['remote'],
): RuneConfig['remote'] | undefined {
  ...
  if (!handler) {
    if (remote.logLevel !== undefined) {
      throw new Error('[rune] remote.logLevel requires a remote.handler (or inherited handler via child())');
    }
...
export function createLogger(initial: RuneOptions | string = {}, initialBindings: Bindings = {}): Logger {
  ...
  const emit = (type: LogType, msgOrCtx?: string | Bindings | Error, msg?: unknown): void => {
    ...
    Promise.resolve()
      ...
      .catch((err: unknown) => {
        console.warn('[rune] remote handler error:', err);
      });
  };
