...
export const HeadlessError = {
  MISSING_BOUNDARY: 'SIGIL_MISSING_BOUNDARY',
  MISSING_HOST: 'SIGIL_MISSING_HOST',
  MISSING_PANEL: 'SIGIL_MISSING_PANEL',
  MISSING_REFERENCE: 'SIGIL_MISSING_REFERENCE',
  MISSING_ROLE: 'SIGIL_MISSING_ROLE',
} as const;
...
export class HeadlessException extends Error {
  ...
  constructor(code: HeadlessErrorCode, message: string) {
    super(import.meta.env.DEV ? `[block ${code}] ${message}` : `[block ${code}]`);
    this.code = code;
