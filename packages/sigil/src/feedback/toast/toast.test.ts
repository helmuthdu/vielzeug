import { FLUSH_DEEP, type Fixture, mount, user, waitFor } from '@vielzeug/craft/testing';
...
// Find the close button inside the first sg-alert's shadow DOM.
...
describe('sg-toast', () => {
  ...
  describe('max eviction', () => {
    it('evicts oldest toast with animation when max is exceeded', async () => {
      fixture = await mount('sg-toast', { attrs: { max: '2' } });
    ...
    it('fires dismiss event for evicted toasts', async () => {
      fixture = await mount('sg-toast', { attrs: { max: '1' } });
  ...
  describe('update()', () => {
    it('updates message in place', async () => {
      ...
      expect(fixture.query('sg-alert')?.textContent?.trim()).toContain('New');
    });
...
describe('sg-toast accessibility', () => {
  ...
  describe('Close button', () => {
    ...
    it('toasts with urgency assertive route to assertive live region', async () => {
      ...
      expect(assertiveRegion?.querySelector('sg-alert')).toBeTruthy();
    });
  });
});
...
describe('createToastService', () => {
  ...
  it('re-uses an existing mounted sg-toast inside the root', async () => {
    // mount() produces a fully-initialized sg-toast (onMounted has fired).
    fixture = await mount('sg-toast');

    const container = fixture.element.parentElement!;
    const countBefore = container.querySelectorAll('sg-toast').length;
    ...
    // No new sg-toast was created.
    expect(container.querySelectorAll('sg-toast').length).toBe(countBefore);
  });
  ...
  it('two independent services pointing at different containers are isolated', async () => {
    ...
    const fixtureA = await mount('sg-toast', { container: containerA });
    const fixtureB = await mount('sg-toast', { container: containerB });
    ...
    // Each service's toast only appears in its own sg-toast's shadow root.
    const shadowA = containerA.querySelector('sg-toast')?.shadowRoot;
    const shadowB = containerB.querySelector('sg-toast')?.shadowRoot;
...
describe('sg-toast accessibility', () => {
  ...
  describe('Accessibility', () => {
    it('passes axe checks', async () => {
      fixture = await mount('sg-toast-provider');
