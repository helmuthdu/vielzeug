...
export type SgCopyCommandProps = {
  /** Border radius size */
  rounded?: RoundedSize;
  /** Component size */
  size?: ComponentSize;
...
export type SgCopyCommandEvents = {
 ...
 *
 * <sg-copy-command value="npx -y @vielzeug/codex" size="sm" variant="bordered"></sg-copy-command>
 *
 * <!-- With a suffix slot (e.g. a cycle button) -->
 * <sg-copy-command value="npm install @vielzeug/ripple">
 *   <button slot="suffix" type="button" aria-label="Next package">›</button>
 * </sg-copy-command>
 * ```
 */
export const COPY_COMMAND_TAG = 'sg-copy-command' as const;

define<SgCopyCommandProps, SgCopyCommandEvents>(COPY_COMMAND_TAG, {
  props: {
      ...
      <div class="wrapper" part="wrapper">
        <button class="command" part="command" type="button" :aria-label="${btnLabel}" @click=${handleCopy}>
          ...
          <span class="copy-icon" part="copy-icon" aria-hidden="true">
            ${() =>
              copied.value
                ? html`<sg-icon name="check" size="14" class="icon-copied" aria-hidden="true"></sg-icon>`
                : html`<sg-icon name="copy" size="14" aria-hidden="true"></sg-icon>`}
          </span>
        </button>
