...
- **`analyse` / `feature` mode** — prefer `mcp0_get-source` (`packageSlug: "<name>"`) over reading files one-by-one. Follow with `mcp0_get-docs` for documented behaviour.
...
After three passes, write `plan.md` to `.ai/workflows/runs/<name>/plan.md`. Read the root `AGENTS.md` first. **Always overwrite** — never append; historical plans are recoverable from git.
