...
- **Overwritten per cycle**, not appended forever. A "cycle" is one fresh `/pkg-workflow` invocation for a package. See `.ai/rules/agent-execution.md § Run artifacts` for the exact overwrite/append rule per file.
