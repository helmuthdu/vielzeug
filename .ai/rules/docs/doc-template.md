...
- **Minimum Node version** is folded into the `node` entry of the environment badge automatically (`Node ≥18`, etc.), sourced from the package's `engines.node` in `package.json` (via `getPackagesData()`) — not from frontmatter, and there is nothing to add to `index.md`. Every package declares `engines.node`; the monorepo-wide floor is `>=18` (first LTS with full ES2022 support), raised per-package only when a real constraint requires it (e.g. `codex` `>=22`, a CLI). There is no equivalent Deno minimum-version — no verified source of truth exists for it, so don't invent one; the `deno` entry in `environments` stays version-less.
