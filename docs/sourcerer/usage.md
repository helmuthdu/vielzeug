...
description: Practical usage patterns for local and remote sources in @vielzeug/sourcerer.
---
...
```ts
import { encodeLocalQueryParams } from '@vielzeug/sourcerer';
...
```ts
import { subscribeSelector } from '@vielzeug/sourcerer';
...
sourcerer exposes framework-agnostic primitives: `subscribe(listener)`, stable
...
Use Courier as the transport layer inside `createRemoteSource()` for consistent retries and error handling.
