...
await source.patch({ filter: filterContains(p => p.category, 'peripherals') })
console.log('Peripherals:', source.current.map(p => p.name))

// filterEquals — strict equality on a field
await source.patch({ filter: filterEquals(p => p.category, 'audio') })
console.log('Audio:', source.current.map(p => p.name))

// filterRange — inclusive min/max on a numeric field
await source.patch({ filter: filterRange(p => p.price, { min: 50, max: 200 }), sort: sortBy(p => p.price, 'asc') })
console.log('$50–$200 asc:', source.current.map(p => \`\${p.name} (\\$\${p.price})\`))

// sortBy descending
await source.patch({ filter: undefined, sort: sortBy(p => p.price, 'desc') })
...
await eventSource.patch({ filter: filterRange(e => e.date, { min: new Date('2024-01-01'), max: new Date('2025-12-31') }) })
