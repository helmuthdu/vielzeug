...
`@vielzeug/clockwork` is a zero-dependency typed finite state machine. Define states, events, transitions, and async invokes once, then get a reactive, fully-typed machine instance with persistence, tracing, and debugging hooks—all with zero runtime overhead.
