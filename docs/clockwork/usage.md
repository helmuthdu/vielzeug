...
```ts
import { assign } from '@vielzeug/clockwork';
...
```ts
import type { ActionArgs } from '@vielzeug/clockwork';
...
Use `@vielzeug/herald` to bridge machine transitions to a shared event bus — useful for cross-machine coordination:
