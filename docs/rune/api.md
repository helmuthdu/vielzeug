...
description: API reference for @vielzeug/rune exports, logger methods, and configuration types.
---
...
| `Rune.scope()`  | Create namespaced logger children | Sync           | Namespace too broad makes filtering noisy          |
...
| Import            | Purpose                                                      |
| ----------------- | ------------------------------------------------------------ |
| `@vielzeug/rune` | Main API (`createLogger`, `Rune`) and exported logger types |

## createLogger(initial?)

Creates an isolated logger instance.

Signature: `createLogger(initial?: RuneOptions | string): Logger`
...
`Rune` is the pre-created default logger instance (`createLogger()` called once).
