---
title: 'Wired Examples — Async Providers'
description: 'Async providers example for @vielzeug/wired.'
---
...
Return a `Promise` from the `factory()` callback. Wired awaits it automatically. Concurrent callers for singleton and scoped lifetimes share the same in-flight promise, so the work runs exactly once.
