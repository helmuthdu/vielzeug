...
```ts
import { defaults, diff, getPath, parseJSON, pick, omit, mapValues, prune, hash } from '@vielzeug/arsenal';
...
- Use `hash` to generate deterministic cache keys from complex option objects.
