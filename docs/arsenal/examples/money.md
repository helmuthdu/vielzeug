...
Implement overview in a production-friendly way with `@vielzeug/arsenal` while keeping setup and cleanup explicit.
...
```ts
import { currency } from '@vielzeug/arsenal';
...
```ts
import { exchange } from '@vielzeug/arsenal';
