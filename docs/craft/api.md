...
description: Entry points and public API reference for @vielzeug/craft, @vielzeug/craft/controls, @vielzeug/craft/observers, and @vielzeug/craft/testing.
---
...
| `@vielzeug/craft/controls`  | Headless interaction controls                      |
| `@vielzeug/craft/observers` | Resize, intersection, and media-query observers    |
...
Component-aware wrapper around ripple's `effect()`. Cleanups registered inside the effect are tied to the current Craft runtime.
...
Adds an event listener and automatically removes it on cleanup when called inside component setup or another Craft runtime scope.

If you call it outside a runtime scope, Craft returns a manual cleanup function and emits a one-time warning in development.
...
Re-exported from `@vielzeug/ripple`. Creates an isolated cleanup context. Use inside component `setup()` to manage sub-scoped teardown independently from the component lifetime.
...
Craft does not expose `defineProps()`. Define props directly in `props` using `prop.*` helpers or raw `PropDef` objects.
...
Import from `@vielzeug/craft/controls`.
