---
title: 'Deposit Examples — Batch Writes'
description: 'Atomic multi-table writes and deferred observer notifications with batch() in @vielzeug/deposit.'
---
...
- `batch()` is table-scoped. Accessing a table inside the callback that was not declared in the first argument throws `DepositScopeError` at runtime and is a type error at compile time.
...
- The `tables` array must not be empty. Passing an empty array throws `DepositScopeError`.

### Related

- [CRUD](./crud.md)
- [Reactive Tables](./reactive.md)
- [Usage Guide — Batch Writes](/deposit/usage.md#batch-writes)
- [API Reference — TransactionContext](/deposit/api.md#transactioncontext)
