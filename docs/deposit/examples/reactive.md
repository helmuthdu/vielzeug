---
title: 'Deposit Examples — Reactive Tables'
description: 'Subscribe to table changes with observe, watch, observeMany, and signals in @vielzeug/deposit.'
---
...
Wire a `@vielzeug/ripple` signal at construction time. Deposit keeps it in sync automatically — no `observe()` call needed.
...
- `observeMany()` throws `DepositScopeError` when passed an empty `tables` array. Always provide at least one table name.
...
- [CRUD](./crud.md)
- [Ripple](/ripple/)
- [Usage Guide — Reactive Reads](/deposit/usage.md#reactive-reads)
