...
description: Use Permit RBAC with Machinit to gate state transitions based on user permissions.
---
...
Use Permit as the source of truth for authorization, and call permit predicates in Machinit guards. The machine enforces the happy path, while permit ensures users can only take allowed actions.

```ts
import { defineMachine, interpret, assign, MachinitError } from '@vielzeug/machinit';
...
- [Stateit](../stateit/) - Reactive signals for user role synchronization
- [Logit](../logit/) - Structured logging for audit trails
