...
description: Use Sourceit with Machinit to manage async data loading, pagination, and search state.
---
...
Use Machinit for overall state machine (idle → loading → success/error), and Sourceit for data source management with pagination and filtering. The machine orchestrates the state, while sourceit handles fetching and caching.

```ts
import { defineMachine, interpret, assign } from '@vielzeug/machinit';
import { createSource } from '@vielzeug/sourceit';
...
3. **Cache not invalidated on filter change** - Sourceit caches results per query. If search query changes but params look identical, stale data may be returned. Clear cache when filters change: `source.invalidate()`.
...
- [Sourceit](../sourceit/) - Reactive data sources with pagination
- [Fetchit](../fetchit/) - HTTP client with caching
