...
description: Use Eventit to coordinate multiple independent Machinit state machines.
---
...
Use Eventit as a message bus to decouple machines. Each machine publishes events about state changes, and other machines subscribe and react accordingly. This creates a pub/sub pattern where machines are independent but coordinated.
...
5. **Hard to debug multi-machine state** - When machines coordinate, debugging requires tracking all states simultaneously. Enable debug hooks on each machine and log to a central registry: `debug: { onTransition: (info) => log({ machine: 'session', ...info }) }`.

## Related

- [Eventit](../eventit/) - Typed event bus and pub/sub
- [Stateit](../stateit/) - Reactive signals for UI synchronization
- [Logit](../logit/) - Structured logging for multi-machine coordination traces
- [Permit](../permit/) - Permission checks during multi-machine workflows
