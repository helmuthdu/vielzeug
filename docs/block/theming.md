---
title: Block — Theming & Customization
description: Design tokens, dark mode, CSS custom properties, and theme variants for Block.
---
...
Block is designed to be highly customizable through CSS Custom Properties (CSS variables). All design tokens are defined in a central theme file that you can reference and override.

## Design Tokens

Block provides a comprehensive set of design tokens organized into the following categories:
...
<<< @/../packages/block/src/styles/theme.css
:::

## Contrast Scale

Block uses a 10-step contrast scale driven entirely by `light-dark()`. The values flip automatically between the light and dark poles — no `@media` query needed.
...
Block uses two parallel font-size scales to cleanly separate body text sizing from display heading sizing.
...
Block uses the CSS [`light-dark()`](https://developer.mozilla.org/en-US/docs/Web/CSS/color_value/light-dark) function to define every color token once with both a light and dark value. `color-scheme: light dark` on `:root` means the OS preference is respected automatically — no `@media` block required.
...
VitePress automatically adds `.dark` to `<html>` when the user toggles the theme. Block responds to this automatically — no extra configuration needed.
...
Block provides pre-built transition and animation tokens. All duration tokens resolve to `0ms` when `prefers-reduced-motion: reduce` is active.
