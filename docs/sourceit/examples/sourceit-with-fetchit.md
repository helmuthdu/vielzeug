---
title: 'Sourceit Examples — Remote Data with Fetchit'
description: 'Use Fetchit as the HTTP transport layer inside a createRemoteSource fetch function.'
---
...
- Fetchit's `api.get()` throws on HTTP errors (4xx/5xx). Wrap the call in try/catch inside the `fetch` callback and return `{ items: [], total: 0 }` so Sourceit can surface its error state rather than crashing.
