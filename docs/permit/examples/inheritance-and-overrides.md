...
description: 'Priority and overrides example for @vielzeug/permit.'
---
...
Use `priority` to ensure the deny rule wins. Permit picks the rule with the highest priority; for equal priority it prefers higher specificity, then deny over allow.
...
permit.can({ id: 'u2', roles: ['staff'] },              'posts', 'read'); // true
```
...
- When priority ties, Permit prefers the more specific rule (non-wildcard fields score higher). If specificity also ties, deny wins over allow. Design your priority scale so that intentional overrides never rely on the deny-tiebreak.
