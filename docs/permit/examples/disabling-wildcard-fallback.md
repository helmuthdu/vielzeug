---
title: 'Permit Examples — Disabling Wildcard Fallback'
description: 'Disabling Wildcard Fallback examples for permit.'
---

## Disabling Wildcard Fallback

## Problem

Implement disabling wildcard fallback in a production-friendly way with `@vielzeug/permit` while keeping setup and cleanup explicit.
...
permit.check({ id: '1', roles: ['admin'] }, 'posts', 'write'); // true
permit.check({ id: '1', roles: ['admin'] }, 'posts', 'read'); // false
```
