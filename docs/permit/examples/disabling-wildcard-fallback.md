...
description: 'Rule specificity example for @vielzeug/permit.'
---
...
At equal priority, Permit picks the more specific rule. An exact resource (`'posts'`) scores higher than `WILDCARD`, so the specific deny wins without any explicit priority setting.
