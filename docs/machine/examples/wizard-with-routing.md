...
description: Sync a multi-step wizard state machine with URL routing using Machine and Route.
---
...
Use Machine to manage wizard steps with Route to sync the current step to the URL. The machine guards validate transitions and the router handles URL changes.

```ts
import { defineMachine, interpret, assign } from '@vielzeug/machine';
import { createRouter, navigate } from '@vielzeug/route';
...
- [Route](../route/) - Client-side routing with middleware
