...
| `required`        | `boolean`                      | `false`              | Fails constraint validation while blank — a native `<form>` blocks submit and `checkValidity()`/`reportValidity()` return `false` |
