...
const PositiveNumber = s.number().refine((n) => n > 0, () => 'Must be positive');
...
```ts
class WrapperSchema<T extends AnySchema, Mode extends WrapperMode> extends Schema<WrapperOutput<T, Mode>, WrapperInput<T, Mode>> {
  readonly inner: T;
  readonly mode: Mode;
